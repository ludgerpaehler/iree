...
// Demonstrates a standalone plugin exporting a single `import_mul` function.
// This models kernel libraries and custom intrinsics where there's only
// stateless functions. Standalone plugins can be compiled to portable ELFs that
// allow the same plugin file to be used on all platforms (linux/windows/mac/
// bare-metal, etc) without the need to recompile or have platform-specific
// toolchains. As much as possible plugins should try to be in this form - when
// getting called in the CPU task system performing syscalls, blocking, or using
// TLS are either unsupported or extremely bad ideas. System linked plugins
// allow all those things but don't make it any safer.
//
...
static int simple_mul_workgroup(void* context, void* params_ptr,
                                void* reserved) {
  typedef struct {
    // Someone please fix this, please.
    ...
    // ^^^^ all this is garbage and should just be 3 pointers, but MLIR
    size_t dim;
    size_t tid;
  } params_t;
