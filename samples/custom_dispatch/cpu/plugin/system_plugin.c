...
// Demonstrates a system linked plugin exporting a single `import_mul` function
// that also prints to stdout. This is not a great idea but shows how plugins
// can have side-effecting behavior - even if in most cases a standalone plugin
// can be used with much smaller code size and portability.
//
...
// place before we start scheduling work. Deferring will introduce first-run
// delays and require warmup steps. Since only the imports used by the program
// are present and most programs use all imports it's almost always going to be
// better to do things ahead of time.
//
